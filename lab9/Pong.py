...
class FuzzyPlayer(Player):
    def __init__(self, racket: Racket, ball: Ball, board: Board):
        ...
        self.board = board
        x_dist = ctrl.Antecedent(np.arange(0, board.surface.get_width(), 1), 'x_dist')
        y_dist = ctrl.Antecedent(np.arange(0, board.surface.get_height(), 1), 'y_dist')

        speed = ctrl.Consequent(np.arange(0, 51, 1), 'speed')

        x_dist['close'] = fuzz.trimf(x_dist.universe, [0, 0, 100])
        x_dist['medium'] = fuzz.trimf(x_dist.universe, [0, 100, 200])
        x_dist['far'] = fuzz.trimf(x_dist.universe, [100, board.surface.get_width(), board.surface.get_width()])

        y_dist['close'] = fuzz.trimf(y_dist.universe, [0, 0, 150])
        y_dist['medium'] = fuzz.trimf(y_dist.universe, [0, 150, 300])
        y_dist['far'] = fuzz.trimf(y_dist.universe, [150, board.surface.get_height(), board.surface.get_height()])

        speed['stay'] = fuzz.trimf(speed.universe, [0, 0, 20])
        speed['slow'] = fuzz.trimf(speed.universe, [0, 5, 30])
        speed['fast'] = fuzz.trimf(speed.universe, [20, 50, 50])
        ...
        rules = [
            ctrl.Rule(x_dist['close'] & y_dist['close'], speed['slow']),
            ctrl.Rule(x_dist['close'] & y_dist['medium'], speed['slow']),
            ctrl.Rule(x_dist['close'] & y_dist['far'], speed['stay']),
            ctrl.Rule(x_dist['medium'] & y_dist['close'], speed['fast']),
            ctrl.Rule(x_dist['medium'] & y_dist['medium'], speed['fast']),
            ctrl.Rule(x_dist['medium'] & y_dist['far'], speed['slow']),
            ctrl.Rule(x_dist['far'] & y_dist['close'], speed['fast']),
            ctrl.Rule(x_dist['far'] & y_dist['medium'], speed['fast']),
            ctrl.Rule(x_dist['far'] & y_dist['far'], speed['fast']),

        ]

        racket_controller = ctrl.ControlSystem(rules)
        self.decision_maker = ctrl.ControlSystemSimulation(racket_controller)

    def act(self, x_diff: int, y_diff: int):
        velocity = self.make_decision(x_diff, y_diff)
        print("x: ", x_diff, "y: ", y_diff, "velocity: ", velocity)
        self.move(self.racket.rect.x + velocity)

    def make_decision(self, x_diff: int, y_diff: int):
        self.decision_maker.input['x_dist'] = x_diff
        self.decision_maker.input['y_dist'] = y_diff
        self.decision_maker.compute()

        speed = self.decision_maker.output['speed']
        direction = -1
        if x_diff < 0:
            direction = 1
...
if __name__ == "__main__":
    ...
    #
    # player_paddle = Racket(x=width // 2, y=height - 20)
    # player = FuzzyPlayer(player_paddle, ball, board)
